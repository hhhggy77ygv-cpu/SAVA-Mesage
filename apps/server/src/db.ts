...
// ─── Расширение Prisma: прозрачное шифрование сообщений ───────────────
// Шифруем `content` и `quote` перед записью в базу,
// расшифровываем после чтения. База никогда не хранит открытый текст.
...
export const prisma = basePrisma.$extends({
  query: {
    ...
    // Также расшифровываем сообщения, вложенные в Chat-запросы
    chat: {
      async findMany({ args, query }) {
        const results = await query(args);
    ...
    // Расшифровываем сообщения внутри PinnedMessage-запросов
    pinnedMessage: {
      async findFirst({ args, query }) {
        const result = await query(args);
...
function decryptMessageFields(obj: Record<string, unknown> | null): void {
  ...
  // Расшифровываем вложенный ответ, если он есть
...
/** Расшифровывает вложенное поле сообщения на любом объекте (например, `PinnedMessage.message`). */
