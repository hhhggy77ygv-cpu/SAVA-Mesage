...
import { getAppSettings } from '../routes/auth';
...
const RATE_LIMIT_MAX = 50; // max events per window (increased for message forwarding)

const MAX_TIMEOUT = 2_147_483_647; // Max safe setTimeout delay (~24.8 days)

// ─── WebRTC SDP validation ───────────────────────────────────────────
// HTML-escape a string to prevent XSS when SDP is rendered in DOM
...
/** Basic validation for WebRTC SDP offers/answers to prevent malformed data */
function isValidSdp(offer: unknown): boolean {
  if (!offer || typeof offer !== 'object') return false;
  const sdp = offer as Record<string, unknown>;
  ...
  if (typeof sdp.sdp !== 'string' || !sdp.sdp.includes('v=') || !sdp.sdp.includes('o=') || !sdp.sdp.includes('s=')) return false;
  if (sdp.sdp.length > 16 * 1024) return false;
  return true;
}

/** Basic validation for ICE candidates */
function isValidIceCandidate(candidate: unknown): boolean {
  if (!candidate || typeof candidate !== 'object') return false;
  const c = candidate as Record<string, unknown>;
  // Must have candidate string and optionally sdpMLineIndex/sdpMid
  if (typeof c.candidate !== 'string') return false;
  if (c.candidate.length > 500) return false; // unreasonable length
  return true;
}
...
/**
 * Check if any member of a personal chat has blocked the sender.
 * Returns true if the message should be blocked.
 */
async function isBlockedInChat(chatId: string, senderId: string): Promise<boolean> {
  try {
    ...
    // Check if any other member has blocked the sender
    for (const member of chat.members) {
      ...
      if (!user) continue;
      let blocked: string[] = [];
      try { blocked = JSON.parse(user.blockedUserIds || '[]'); } catch { blocked = []; }
      if (blocked.includes(senderId)) return true;
    }
    return false;
  } catch {
    return false;
...
export function setupSocket(io: Server) {
  ...
  io.on('connection', async (socket: AuthSocket) => {
    ...
    // Отправка опроса
    socket.on('send_poll', async (data: {
      chatId: string;
      question: string;
      ...
      try {
        ...
        if (!data.chatId || !data.question?.trim() || !Array.isArray(data.options) || data.options.length < 2) return;
        if (!(await isChatMember(data.chatId, userId))) return;
        if (await isBlockedInChat(data.chatId, userId)) return;
          ...
          data: {
            ...
            poll: {
              create: {
                ...
                options: JSON.stringify(data.options.slice(0, 10).map(o => ({ id: o.id, text: o.text.trim().slice(0, 100) }))),
        ...
        io.to(`chat:${data.chatId}`).emit('new_message', { ...message, readBy: [{ userId }] });
      } catch (e) {
    ...
    socket.on('send_message', async (data: {
      ...
      try {
        ...
        if (await isBlockedInChat(data.chatId, userId)) {
          socket.emit('error', { message: 'Вы не можете отправлять сообщения этому пользователю' });
          return;
        }
        ...
        if (msgType === 'voice' && !settings.enableVoiceMessages) {
          socket.emit('error', { message: 'Голосовые сообщения отключены администратором' });
          return;
        }
        
        // Check if file uploads are enabled
        if (data.mediaUrl && !settings.enableFileUpload) {
          socket.emit('error', { message: 'Загрузка файлов отключена администратором' });
          return;
        }
        
        // Check if forwarding is enabled
        if (data.forwardedFromId && !settings.enableForwarding) {
          socket.emit('error', { message: 'Пересылка сообщений отключена администратором' });
          return;
        }
        
        // Check if scheduled messages are enabled
        if (data.scheduledAt && !settings.enableScheduledMessages) {
          socket.emit('error', { message: 'Отложенные сообщения отключены администратором' });
          return;
        }
        ...
        for (const member of members) {
          // Restore hidden chat for members who deleted it
          await prisma.chatMember.updateMany({
            where: { chatId: data.chatId, userId: member.userId, isHidden: true },
            data: { isHidden: false },
          });
    ...
    socket.on('typing_start', async (chatId: string) => {
      ...
      if (await isBlockedInChat(chatId, userId)) return;
    ...
    socket.on('edit_message', async (data: { messageId: string; content: string; chatId: string }) => {
      try {
        ...
        // Проверяем, что пользователь все еще является участником чата
    ...
    socket.on('delete_message', async (data: { messageId: string; chatId: string }) => {
      try {
        ...
        if (message.senderId !== userId) {
          socket.emit('error', { message: 'Вы можете удалять только свои сообщения' });
          return;
        }
    ...
    socket.on('delete_messages', async (data: { messageIds: string[]; chatId: string; deleteForAll: boolean }) => {
      try {
        ...
        if (data.deleteForAll) {
          // Удалить у всех — только автор может удалить свои сообщения у всех
          ...
          for (const message of messages) {
            // Проверяем, что пользователь является автором сообщения
            if (message.senderId !== userId) {
              socket.emit('error', { message: 'Вы можете удалять у всех только свои сообщения' });
              continue;
            }
    ...
    socket.on('add_reaction', async (data: { messageId: string; emoji: string; chatId: string }) => {
      try {
        ...
        if (!settings.enableReactions) {
          socket.emit('error', { message: 'Реакции отключены администратором' });
          return;
        }
    ...
    socket.on('call_offer', async (data: { targetUserId: string; offer: unknown; callType: 'voice' | 'video'; chatId?: string }) => {
      ...
      if (!isValidSdp(data.offer)) {
        socket.emit('error', { message: 'Некорректный WebRTC offer' });
        return;
      }

      // Block check — don't allow calls to users who blocked the caller
      ...
      if (targetUser) {
        let blocked: string[] = [];
        try { blocked = JSON.parse(targetUser.blockedUserIds || '[]'); } catch { blocked = []; }
    ...
    socket.on('call_answer', (data: { targetUserId: string; answer: unknown }) => {
      if (!isValidSdp(data.answer)) {
        socket.emit('error', { message: 'Некорректный WebRTC answer' });
        return;
      }
      const targetSockets = onlineUsers.get(data.targetUserId);
      if (targetSockets) {
    ...
    socket.on('group_call_offer', (data: { chatId: string; targetUserId: string; offer: unknown }) => {
      if (!isValidSdp(data.offer)) return;
      const targetSockets = onlineUsers.get(data.targetUserId);
      if (targetSockets) {
        for (const sid of targetSockets) {
    ...
    socket.on('group_call_answer', (data: { chatId: string; targetUserId: string; answer: unknown }) => {
      if (!isValidSdp(data.answer)) return;
      const targetSockets = onlineUsers.get(data.targetUserId);
      if (targetSockets) {
        for (const sid of targetSockets) {
    ...
    // ─── Block / Unblock relay ────────────────────────────────────────
    socket.on('user_blocked', (data: { targetUserId: string }) => {
      if (!data.targetUserId || typeof data.targetUserId !== 'string') return;
      const targetSockets = onlineUsers.get(data.targetUserId);
      if (targetSockets) {
        for (const sid of targetSockets) {
          io.to(sid).emit('you_were_blocked', { byUserId: userId });
        }
      }
    });

    socket.on('user_unblocked', (data: { targetUserId: string }) => {
      if (!data.targetUserId || typeof data.targetUserId !== 'string') return;
      const targetSockets = onlineUsers.get(data.targetUserId);
      if (targetSockets) {
        for (const sid of targetSockets) {
          io.to(sid).emit('you_were_unblocked', { byUserId: userId });
        }
      }
    });

    // Periodic lastSeen update (every 30 seconds while connected)
    ...
    // Отключение — объединенный handler
...
async function rescheduleMessages(io: Server) {
  try {
    ...
    for (let i = 0; i < scheduled.length; i++) {
      ...
      // Stagger future messages by 100ms each; overdue messages fire immediately
      const scheduledTime = new Date(msg.scheduledAt!).getTime();
      const now = Date.now();
      const rawDelay = scheduledTime - now;
      const delay = Math.min(Math.max(rawDelay < 0 ? 0 : i * 100, rawDelay), MAX_TIMEOUT);
      setTimeout(async () => {
        try {
          // Check if message was deleted while waiting
