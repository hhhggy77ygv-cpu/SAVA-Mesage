...
import { ipKeyGeneratorIpFallback } from 'express-rate-limit/lib/key-generators/ip.mjs';

const router = Router();

// Rate limit: max 30 friend requests per 10 minutes per user (increased from 10)
const friendRequestLimiter = rateLimit({
  windowMs: 10 * 60 * 1000,
  max: 30,
  message: { error: 'Слишком много заявок. Подождите немного.' },
  ...
  keyGenerator: (req) => {
    ...
    // Use built-in IPv6-aware key generator with fallback
    return ipKeyGeneratorIpFallback(req);
  },
});
...
// ─── Send friend request ─────────────────────────────────────────────
router.post('/request', friendRequestLimiter, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
...
// ─── Get mutual friends ───────────────────────────────────────────────
router.get('/mutual/:targetId', async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    ...
    const [myFriendships, theirFriendships] = await Promise.all([
      ...
      prisma.friendship.findMany({
        where: { status: 'accepted', OR: [{ userId: targetId }, { friendId: targetId }] },
        select: { userId: true, friendId: true },
      }),
    ]);

    const myFriendIds = new Set(myFriendships.map(f => f.userId === userId ? f.friendId : f.userId));
    const theirFriendIds = new Set(theirFriendships.map(f => f.userId === targetId ? f.friendId : f.userId));

    const mutualIds = [...myFriendIds].filter(id => theirFriendIds.has(id) && id !== userId && id !== targetId);
