...
import { deleteUploadedFile, UPLOADS_ROOT } from '../shared';
...
router.post('/', async (req: AuthRequest, res) => {
  try {
    ...
    if (!settings.enableStories) {
      res.status(403).json({ error: 'Истории отключены администратором' });
      return;
    }
    ...
    // Validate mediaUrl — must be under uploads root (path containment check)
    if (mediaUrl) {
      if (typeof mediaUrl !== 'string' || !mediaUrl.startsWith('/uploads/')) {
        res.status(400).json({ error: 'Недопустимый URL медиафайла' });
        return;
      }
      const resolved = path.resolve(UPLOADS_ROOT, mediaUrl.replace(/^\/uploads\//, ''));
      if (!resolved.startsWith(UPLOADS_ROOT)) {
        res.status(400).json({ error: 'Недопустимый путь медиафайла' });
        return;
      }
    }
...
router.post('/:storyId/view', async (req: AuthRequest, res) => {
  try {
    ...
    // Atomic create — if already viewed, unique constraint catches it
