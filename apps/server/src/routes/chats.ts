...
import { USER_SELECT, SENDER_SELECT, uploadGroupAvatar, deleteUploadedFile, encryptUploadedFile, validateFileMagicBytes, resizeAvatar } from '../shared';
...
router.get('/', async (req: AuthRequest, res) => {
  try {
    const chats = await prisma.chat.findMany({
      where: {
        members: { some: { userId: req.userId, isHidden: false } },
      },
      include: {
        members: {
...
router.post('/:id/avatar', uploadGroupAvatar.single('avatar'), validateFileMagicBytes, resizeAvatar, encryptUploadedFile, async (req: AuthRequest, res) => {
  try {
    const settings = getAppSettings();
    if (!settings.enableFileUpload) {
      res.status(403).json({ error: 'Загрузка файлов отключена администратором' });
      return;
    }
    ...
    // Capture old avatar BEFORE updating
    const oldAvatar = await prisma.chat.findUnique({ where: { id: chatId }, select: { avatar: true } });
    ...
    res.json(chat);
    // Delete old avatar only after DB update succeeded
...
// Удалить чат (скрыть для себя — soft delete)
router.delete('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    if (chat.type === 'personal' || chat.type === 'favorites') {
      // Soft delete: скрыть чат и очистить историю для себя
      await prisma.chatMember.update({
      ...
      if (chat.members.length <= 1) {
        // Последний участник — удалить группу полностью
