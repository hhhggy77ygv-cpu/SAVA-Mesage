...
class ApiClient {
  ...
  private async tryRefresh(): Promise<boolean> {
    ...
    this.refreshPromise = (async () => {
      ...
      } catch { /* network error — keep current token */ }
    })();
  ...
  private async request<T>(endpoint: string, options: RequestInit & { timeout?: number } = {}): Promise<T> {
    ...
    const doFetch = async (): Promise<Response> => {
      ...
      try {
        ...
        const url = apiBase.startsWith('http') ? `${apiBase}${endpoint}` : `${apiBase}${endpoint}`;
  ...
  async uploadFile(file: File) {
    ...
    const timer = setTimeout(() => controller.abort(), 120_000);
    const apiBase = getApiBase();
    const url = apiBase.startsWith('http') ? `${apiBase}/messages/upload` : `${apiBase}/messages/upload`;
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        ...(this.token ? { Authorization: `Bearer ${this.token}` } : {}),
